...
void deleteNextStudent(struct Student* previousStudent) {
    if (previousStudent != NULL && previousStudent->next != NULL) {
        struct Student* toDelete = previousStudent->next;
        previousStudent->next = toDelete->next;
        free(toDelete);
    }
}
...
int main() {
    ...
    addStudent(&studentList, 205, "John", 22);

    printStudents(studentList);

    const char* searchName = "Saliha";
    struct Student* previousStudent = searchStudentByName(studentList, searchName);

    if (previousStudent != NULL) {
        deleteNextStudent(previousStudent);
        printf("Deleted the student after '%s'\n", searchName);
    } else {
